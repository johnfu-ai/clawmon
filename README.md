...
1. 启动小程序，保持窗口开着（可以最小化，后台仍会轮询 + 自动继续）。
...
- 同一目录下同时开多个 claude 时，映射到的是该目录下最新的会话记录
  （进程粒度仍正确，只是“最后活动”取最新文件）。
- 长时间运行的工具命令（如 sleep 10 分钟的 Bash）期间空闲时间会增长，
  可能把「处理中」误判为红灯；把「超时判定」调大即可规避。
