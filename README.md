...
- **发现会话**：扫描 `/proc` 中的 claude 进程（PID、工作目录、TTY、是否在
  ...
  关掉 Windows Terminal 标签后，node（Claude Code）会忽略 SIGHUP：pts 仍
  在、进程还活着，但窗口已经没了。这类幽灵会话不列出（pts 已 `(deleted)`，
  或对应的 `wsl.exe` 只剩一个 0×0 的 `PseudoConsoleWindow`）。在 tmux 里
  的会话会保留——那正是 tmux 的用途，关窗口也能再 attach。
- **判断状态**（基于记录里最后一条写入 + 空闲时长）：
  | 灯 | 条件 | 含义 |
  |----|------|------|
  | 🟢 | 空闲 < 活跃阈值（默认 2 分钟） | 正在干活 |
  | 🟢 | 挂起的 tool 是 `Task` / `Agent` / `TaskOutput` | 等待 Subagent（子代理在写自己的记录，主会话静默属正常推进，不算等待输入） |
  | 🟢 | 挂起的 tool 是其它工具 | 工具执行中（等的是工具，不是你） |
  | 🟢 | 最后一条是 user（等待模型响应）但未超阈值 | 等待 API 响应（慢不等于卡） |
  | 🔵 | 最后一条是 assistant、无挂起工具，且后面有 `turn_duration` / `stop_hook_summary` | 回合完成（记录里的收尾才算跑完一轮；单凭一段 assistant 文本不够，模型常在那之后继续思考或再调工具） |
  | 🔵 | 超过活跃窗口仍不是 user / 挂起工具 / AskUserQuestion | 按已完成处理（旧记录没有收尾标记时的兜底；不再标黄「等待输入」） |
  | 🟡 | 挂起的 tool 是 `AskUserQuestion` | 在等你回答问题 |
  | 🟡 | 找不到会话记录 | 刚启动尚未写盘，或进程不属于任何记录 |
  | 🟡 | 记录的写入时间早于进程启动 | 配对到的是一份旧记录，不作数 |
  | 🔴 | 最后一条是 user 且空闲 > 超时阈值（默认 5 分钟） | 疑似 API 超时/卡死 |
  | 🔴 | 最后一条是用量上限 429（`使用上限` / `usage quota`），即使后面有 `turn_duration` | 额度已用尽，等到错误里的重置时间后再自动继续 |

  灯的语义是「要不要你」：**绿灯 = 不需要你**（在干活、等工具、等 Subagent、
  等 API 都算）；**蓝灯 = 回合已完成**，无事发生，只等你回来；**黄灯 = 中途
  卡在等你**（等你回答问题），或**看不出它在等什么**（后两条）——拿不准宁可
  黄灯。后两条同时也是**红灯的前提**：只有确认这份记录确实属于该进程，才会
  判定超时，否则一个刚启动、还没写记录的会话会在第一轮就被判红，甚至几小时
  后被误发一次按键——误判的代价是往一个无关终端里敲回车。
- **自动继续**：会话变红后开始计时。用量上限 429 会从错误文案里解析重置
  时间（例如「限额将在 2026-09-19 16:21:07 重置」），到点后再通过
  `tmux send-keys -t <pane> Enter` 发送继续按键；解析不到重置时间时，退回
  设定等待时长（默认 **5 小时**）。若仍未恢复，每隔一段时间重试（默认
  10 分钟，最多 3 次）。所有参数可在设置里改。每次尝试在调度的那一刻就
  记入档案，因此两次重叠的轮询不会重复发按键；不在 tmux 里的会话不会倒
  计时（显示「不在 tmux 中，无法自动继续」）。
...
4. **桌面宠物**：一只 Claude Code 螃蟹（Clawd）风格的珊瑚橘小螃蟹
   ...
   红黄蓝绿灯（绿=正常眨眼、蓝=开心眨眼并显示蓝色数量徽章、黄=打瞌睡、
   红=发抖并显示数量徽章、WSL 断连=变灰睡觉），两只钳子会交替挥舞；可拖到
   任意位置，点一下
...
- 工具执行期间（最后一条是含 `tool_use` 的 assistant）不会误判红灯，
  显示为绿灯「工具执行中」；但转录无法区分「工具在跑」和「在等你批准
  权限」——后一种也会显示绿灯，需回到终端确认。工具刚结束、模型长时间
  排队响应时，与用量耗尽仍不可区分（绿灯等 API / 红灯超时）。
